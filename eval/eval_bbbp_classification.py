...
"""
BBBP classcomparisonevaluationscriptBaseline LLM vs MolAware+GNN

example
 CUDA_VISIBLE_DEVICES=5 python eval_bbbp_classification.py \
 --cfg configs/epoch2_config_modified.yaml \
 --ckpt_dir ${DATA_DIR:-/path/to/data}/MSMLM/model/llama3.2-chem-sft-gnn/1125_llm/epoch2/refine_ner \
 --test_file ${DATA_DIR:-/path/to/data}/MoleculeNet/qa_data/mol_qa_test.jsonl \
 --max_samples 1000 \
 --out_dir ${DATA_DIR:-/path/to/data}/MSMLM/model/llama3.2-chem-sft-gnn/1123_llm_gnn_loss/epoch2/eval_bbbp_compare \
 --temperature 0.0
"""
...
def extract_bbbp_label_from_text(text: str) -> Optional[int]:
 """
 modelgenerate BBBP classlabel
 - match 0 / 1
 - otherwisekey heuristic
 1: can / permeable / crosses BBB / yes
 0: cannot / non-permeable / does not cross / no
 """
 ...
 # 3) yes / no heuristicif prompt yes/no
...
def compute_classification_metrics(y_true: List[int], y_pred: List[int]) -> Dict[str, float]:
 """
 computeclassmetricAccuracy / Precision / Recall / F1
 """
...
def prepare_bbbp_indices(dataset, max_samples: Optional[int]) -> List[int]:
 """
 BBBP classification sample index max_samples 
 constraint
 - dataset == "BBBP"
...
def run_one_inference_bbbp(
 ...
 """
 user_text generatereturns (caption, parsed_label)
 """
 if not user_text:
 return "", None
...
def eval_bbbp_compare(
 ...
 2) with_gnn: prompt <mol>{smiles}</mol>realtime_mol=True

 setsuccessparse 0/1 samplemetric
 """
 ...
 # ---------- baseline promptoriginal input ----------
 # original input alreadytaskdescription SMILES 
 # ---------- with_gnn promptappend <mol>{smiles}</mol> ----------
 if smiles:
 gnn_user_text = (
 base_user_text
 + "\n\nMolecule SMILES: <mol>"
...
def parse_args():
 ...
 help="maxevaluationsample BBBP classification sample")
 parser.add_argument("--out_dir", type=str, required=True,
 help="outputmetric JSON directory")
